    ...
    for i, (images, labels, indexes) in enumerate(train_loader):
        ...
        if (i+1) % args.print_freq == 0:
            print ('Epoch [%d/%d], Iter [%d/%d], Loss1: %.4f, Loss2: %.4f, Pure Ratio1: %.4f, Pure Ratio2 %.4f'
                  %(epoch+1, args.n_epoch, i+1, len(train_dataset)//batch_size, loss_1.data[0], loss_2.data[0], np.sum(pure_ratio_1_list)/len(pure_ratio_1_list), np.sum(pure_ratio_2_list)/len(pure_ratio_2_list)))
        '''
...
def main():
    ...
    batch_size = 5
    # part_train = torch.utils.data.random_split(train_dataset, [100, len(train_dataset) - 100])[0]
    # part_test = torch.utils.data.random_split(test_dataset, [10, len(test_dataset) - 10])[0]
